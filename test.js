...
export default function Message({
  ...
  useEffect(() => {
    ...
    async function searchIfBlocked() {
      ...
      if (userType === "client") {
        // data = await searchClientFavorite(comp?._id, user._id);
        if (!userInfos._id && !user._id) return;

        data = await searchUserBlocked(userInfos._id, user._id);
        console.log(data, "blocked", userInfos._id, user._id);
        if (data.found) {
          console.log(data);
          setIsBlocked(true);
  ...
  useEffect(() => {
    async function getConversation() {
      if (userType === "client") {
        ...
        console.log(conversation, projectClient);
            ...
            {isBlocked ? (
              <div>
                ...
                {blocageType === userType
                  ...
                  : "Vous etes bloqué par cet utilisateur"}{" "}
              </div>
            ) : !voiceMessage ? (
